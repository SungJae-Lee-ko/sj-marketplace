...
name: news-video-gen
description: >
  [책임 경계] 뉴스 쇼츠 영상 생성 오케스트레이터. news-storyboard의 씬별 콘티를 받아 씬 스틸 생성 → image-to-video 변환 → 클립 검수까지 생성 실행 체인을 담당한다. 프롬프트 설계는 news-storyboard, 생성 실행은 본 스킬. "영상 생성해줘", "이 스토리보드로 클립 뽑아줘", "AI로 씬 만들어줘" 요청 시 반드시 사용. 뉴스형 전용 규칙으로 **화면 글자·수치·로고를 AI에게 그리게 하지 않고**(자막 레이어로 이관), 실존 인물·언론사 지면 생성을 차단하며, 인물 씬은 캐스팅 시트 기반 앵커 스틸 → i2v 필수 체인으로 일관성을 보장한다. 씬 역할별 모션 프리셋 자동 매핑 + 생성 전 비용 추정 고지 + 플랜 제약 모델 폴백을 지원하고, 결과를 조립 지시서(JSON)로 산출한다.
version: "0.1.0"
---

# 뉴스 쇼츠 영상 생성 (Video Generation Orchestrator)

**책임 한 줄**: 확정된 스토리보드 + 캐스팅 시트 → 씬별 [스틸 생성 → image-to-video] 체인 실행 → 검수된 클립 세트 + 조립 지시서 반환.
...
1. news-storyboard 산출물이 확정됨. **씬 표의 10개 고정 컬럼(#·시간·화면 구성·앵글/무빙·role·나레이션·자막·출처 크레딧·오디오·person_group)과 씬 시간표가 모두 있어야 한다**
...
6. **생성 전 비용 추정 고지 + 사용자 승인** — 승인 없이 생성 호출 금지
7. **입력 소재**
...
[Step 0] 인물 일관성 결정 (인물 등장 시 필수 — 아래 의사결정 트리)
   ↓
[Step 1] 씬 스틸 이미지 생성  → 파일명 규칙: still_scene{N}.png (인물 앵커는 still_{P그룹}_anchor.png)
   └─ 인물 씬: 캐스팅 시트 → 그룹별 앵커 스틸 → 이후 씬은 앵커 참조로
   └─ data_card 씬: **배경만.** 글자·숫자·그래프를 그리지 않는다
   └─ context 씬: 톤 참고의 무드 묘사를 프롬프트로
   └─ demo 씬(guide): 시작·정점 2스틸
   ↓
[Step 2] image-to-video 변환
   └─ Step 1 이미지를 start_image(첫 프레임) 소스로, 모션 프리셋 적용
   └─ ⚠️ 인물 등장 씬은 이 경로가 유일한 허용 경로 — 참조 이미지만 넣은 text-to-video 직행 금지
      (참조는 인물을 매 생성마다 새로 발명해 클립 중간에 사람이 바뀌는 identity switching이 발생한다)
   └─ **클립 길이 규칙**: 생성 클립 길이는 **씬 배정 초를 덮는 최소 단위**로 잡는다.
      배정 초가 모델 최소 단위(통상 5초)를 넘으면(5.5~6초 씬) 다음 단위(10초)로 올리거나
      멀티컷으로 분할한다. **배정 초보다 짧은 클립을 만들지 않는다** — 조립에서 빈 구간이 생긴다
   ↓
[Step 3] 클립 검수
   └─ 공통: 손가락·관절 왜곡 / 씬 목적 부합 / 자막·크레딧 얹힐 하단 여백 확보
   └─ 뉴스형 필수: **화면에 글자·숫자·로고가 생성되지 않았는가** / 실존 인물처럼 보이는 얼굴이 없는가
   └─ 인물 씬: 클립 중간 인물 교체 / 같은 person_group 씬 간 동일 인물 유지
   └─ 실행 환경이 클립 파일을 직접 열 수 없으면 검수 항목을 사용자에게 명시적으로 제시하고 확인받는다
   └─ 불합격 시 재생성 1회 (프롬프트 수정 후) — 2회 실패면 실촬영·정지 이미지 전환 제안
...
## 모션 프리셋 (스토리보드 `role` 기준 자동 매핑)

`role`은 **스토리보드가 소유하는 값**이다. 여기서 새로 만들지 않고 받아서 매핑만 한다.

| `role` | 프리셋 | 이유 |
|---|---|---|
| `hook` | impact_zoom 또는 reveal | 첫 프레임에서 시선 고정 |
| `context` 상황·b-roll·브리지 | pan 또는 handheld | 생활감 |
...
- 복잡한 행위를 영상 프롬프트로 연출하라고 요구하지 않는다 — **행위의 정점을 Step 1 스틸에 고정**하고 영상은 카메라 무빙·미세 모션만 입힌다
...
- **a. 씬 내 일관성 — 스틸 고정**: 인물 등장 씬은 반드시 Step 1 스틸 → start_image i2v 체인으로 생성한다
- **b. 씬 간 일관성 — 앵커 스틸 재사용**: **스토리보드의 `person_group` ID가 기준이다.** 같은 그룹의 첫 인물 씬 스틸을 앵커로 삼고, 이후 씬 스틸 생성 시 앵커를 인물 참조로 함께 전달해 "same person as the reference image, different pose/angle"로 지시한다. **`P1`의 앵커를 `P2` 씬에 쓰지 않는다**
...
| 오류 | 대응 |
|---|---|
| 플랜 제한 | 같은 계열 경량 모델 → reference 지원 타 벤더 모델 순으로 폴백. 단가를 다시 preflight |
...
📊 생성 비용 추정
- 씬 수: N개 (스틸 N장 + 클립 N개)
- **data_card·caution 씬은 정지 스틸 + 조립 켄번스로 대체 가능** — 클립 비용 없음. 이 대안을 함께 제시
- 배정 초가 5초를 넘는 씬은 상위 클립 단위(10초)로 계산
- (guide) 동작 시연 씬은 씬당 스틸 2장·클립 2개, 배정 4초 이상이면 3컷 + 그룹별 앵커 스틸 1장 추가
- 재생성 예비: +20%
- 예상 크레딧/비용: [연결된 MCP 단가 기준]
진행할까요? (훅 씬 1개 테스트 생성 후 나머지 일괄을 권장합니다)
```

## 출력 형식 (조립 지시서)

**이 지시서는 news-assemble의 입력 데이터다.** 필드명을 임의로 바꾸거나 빼지 않는다.
...
```json
{
  ...
  "clips": [
    {"scene": 1, "cut": 1, "cut_seconds": 4.0,
     "role": "hook", "duration": 5, "trim_to": "0.0-4.0",
     "motion": "impact_zoom", "source": "still_scene1.png", "url": "...",
     "person_group": "P1", "person_anchor": "still_p1_anchor.png",
     "narration": "계단 내려갈 때 무릎 시큰한 분", "narration_file": null,
     "source_credit": null,
     "review": "pass", "note": null},
    {"scene": 2, "cut": 1, "cut_seconds": 2.0,
     "role": "context", "duration": 5, "trim_to": "0.0-2.0",
     "motion": "pan", "source": "still_scene2.png", "url": "...",
     "person_group": null, "person_anchor": null,
     "narration": null, "narration_file": null,
     "source_credit": null,
     "review": "pass", "note": "브리지 씬 (나레이션 없음)"},
    {"scene": 3, "cut": 1, "cut_seconds": 6.0,
